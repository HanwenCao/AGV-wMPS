...
import math
from geometry_msgs.msg import Pose, Quaternion, TransformStamped, Vector3, Point, PoseArray
import tf2_ros
...
class DemoResetter():
    def __init__(self):
        rospy.init_node('Prototype_get_trans_position')
        ...
        # self.publishMap()
        self.pub_transmitter = rospy.Publisher('/transmitters', PoseArray, queue_size=10)
        ...
        self.center_ray = []
        self.result_ray = []
        self.p_trans1 = []  # remember positions of transmitter 1
        self.p_trans2 = []  # remember positions of transmitter 2
        ...
        self.output_array = np.zeros((2,3))  # 2 transmitters' positions [x,y,z] in robot (6d target) frame
        self.initialGoals()
        self.navigate()
    ...
    def getcurrentpose(self):
        ...
        print 'pose_map:', self.pose_map
        # for coordinates calculation
        q = self.pose_map[0:2]
        theta = self.pose_map[2]
        #R = np.array([[math.cos(theta), -math.sin(theta)],[math.sin(theta), math.cos(theta)]])
        R = np.array([[np.cos(theta), np.sin(-theta)],[np.sin(theta), np.cos(theta)]])  # robot->map
        T = np.copy(q).reshape((2,1))
        

        # then p = R*p_robot + q, where p_robot is position in robot frame, and p is position in map frame
        
        # read from socket: position os transmitters in robot frame
        p_robot = self.output_array[0:2,0:2].T  # p_robot---col1:[x1,y1], col2:[x2,y2];  only need output_array[0:2], which are x and y
        
        p = np.dot(R, p_robot) + q.reshape((2,1))  # col1:[x1,y1], col2:[x2,y2]
        p = p.T   # row1:[x1,y1], row2:[x2,y2]

        print 'positions in map:',p
        self.p_trans1.append(p[0,:])
        self.p_trans2.append(p[1,:])
        '''
        #self.pose_map[2] = self.pose_map[2]*180/math.pi  #nparray[x,y,theta(degree)]
        p = np.dot(np.linalg.inv(R), (p_robot - T))  # position of transmitters in map frame
        print p_robot
        print T
        print p_robot - T
        '''
        #print p
        # Now publish the positions over topic '/transmitters'
        ...
        position_msg.poses = [p1,p2]
        
        self.pub_transmitter.publish(position_msg)  # publish to ros topic
        np.savetxt('transmitter1_positions.txt', self.p_trans1, delimiter=',') 
        np.savetxt('transmitter2_positions.txt', self.p_trans2, delimiter=',') 

        # tar->transmitter i
        R_1 = self.output_array[2:5,:]
        T_1 = self.output_array[5,:].reshape(3,1)
        R_2 = self.output_array[6:9,:]
        T_2 = self.output_array[9,:].reshape(3,1)

        #print "R1", R_1 

        R_mb = np.array([[np.cos(theta), np.sin(theta), 0],[-np.sin(theta), np.cos(theta), 0], [0,0,1]])
        q_3d = np.array([q[0], q[1], 0]).reshape((3,1)) #T_mb
        T_mb = -np.dot(R_mb, q_3d).reshape((3,1))
        R_btar = np.identity(3)  
        T_btar = np.array([0,0,-0.415]).reshape((3,1))  # hight roughly is 0.415
        R_mtrans = np.dot(np.dot(R_2, R_btar), R_mb)

        T_mtrans = np.dot(np.dot(R_2, R_btar), T_mb) + np.dot(R_2, T_btar) + T_2
        # then, R_mtrans*pm + T_mtrans = p_trans, where p_trans is all the positions of receivers for calibration

        # map->transmitter 2
        R2T2 = np.concatenate((R_mtrans, T_mtrans.reshape(1,3)), axis=0)
        np.savetxt('R2T2.txt', R2T2, delimiter=',')
    ...
    def navigate(self):
        ...
        while not rospy.is_shutdown():   
            if not self.stop:
                try:
                    output = conn.recv(30*8)
                    if output.strip() == "disconnect":
                        conn.close()
                        sys.exit("Received disconnect message.  Shutting down.")
                        conn.send("dack")
                    elif output:
                        self.output_array = np.frombuffer(output).reshape(2+8,3)   # positions of 2 transmitters in robot (6d) frame, col=xyz
                        print 'receive:\n', self.output_array
    ...
    def process(self, info, grid, odom):
        ...
        #--------------------------------------------- closing ---------------------
        # operation on open cell (For RPlidar, there could be many holes in the free space)
        thresh_low = 0
        thresh_high = 10
        dst =((grid <= thresh_high) & (grid >= thresh_low))*1.0   
        # "1" in dst are the open cells
        # "0" in dst are the unvisited cells and occupied cells
        
        dst_c=sm.closing(dst,sm.disk(9)) # closing
        grid[dst_c == 1] = 0  # "fill in the holes"
        ...
        print "found %d (including trivial) frontiers" %(label_num)  #added this line

        manh_dist = []  # stores distances
        cents = []  # stores centers of frontiers
        for region in regionprops(conected_frontier):
            # take regions with large enough areas           
            if region.area >= 10:  # do not consider small frontier groups  (set 100 for large rooms)
        ...
        for center in cents_sorted:  # in image frame
            ...
            if (c in self.center_ray):  # if it has been already calculated
                ...
                print "raycasting at this position: ", center
                num_unknown = self.ray_casting(size, unknown, occup, center, 250)  # ray casting
        ...
        utility = np.asarray(entropy_shannon) - np.asarray(entropy_renyi)  # full utility
        ...
        if self.flag_stuck==0:
            ...
            next_goal_world = cents_sorted_world[0]  # closest frontier
