...
int main(int argc, char** argv){
	...
	//for(int i=0;i<256;i++)
        ...
        //finish reading
        //process
        string ang_x, ang_y, ang_z;
        string pos_x, pos_y, pos_z;
        size_t found_x, found_y, found_z;

        found_x = STRING.find('X');
        found_y = STRING.find('Y');
        found_z = STRING.find('Z');

        ang_x = STRING.substr(found_x+3, 10);
        //cout << ang_x<<endl;
        ang_y = STRING.substr(found_y+3, 10);
        //cout << ang_y<<endl;
        ang_z = STRING.substr(found_z+3, 10);
        //cout << ang_z<<endl;

        found_x = STRING.find('X', found_x+1);
        found_y = STRING.find('Y', found_y+1);
        found_z = STRING.find('Z', found_z+1);
        pos_x = STRING.substr(found_x+3, 10);
        //cout << pos_x<<endl;
        pos_y = STRING.substr(found_y+3, 10);
        //cout << pos_y<<endl;
        pos_z = STRING.substr(found_z+3, 10);
        //cout << pos_z<<endl;
        //x = ::atof(pos_x.c_str());
        x = atof(pos_x.c_str());//string to float
        y = atof(pos_y.c_str());
        z = atof(pos_z.c_str());
        xth = atof(ang_x.c_str());
        yth = atof(ang_y.c_str());
        zth = atof(ang_z.c_str());
